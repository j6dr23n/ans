...
class UploadToStreamSB implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle()
    {
        ...
        $upload_url = $this->getUploadUrl($setting->streamsb_api_key); //get upload url from streamsb api
        $results = $this->uploadVideoDD($setting->streamsb_api_key, $upload_url); // start uploading to streamsb account
        Log::info($upload_url, $results);
        $code = $results['result'][0]['code'];

        //Episode Info Updated
        $episode = Episode::where('id', $this->id)->firstOrFail();
        if ($episode->embed_link === null) {
            $episode->drive = 'streamsb';
            $episode->epi_link = "https://sblongvu.com/d/".$code.".html";
            $episode->epi_720p_link = "https://sblongvu.com/d/".$code.".html";
            $episode->embed_link = "https://sblongvu.com/e/".$code.".html";
            $episode->save();
        }
        ...
        if ($episode->embed_link !== null) {
            MoreDownload::create([
                ...
                'drive' => 'streamsb',
                'resolution' => '720p',
                'download_link' => "https://sblongvu.com/d/".$code.".html"
            ]);
            ...
            MoreEmbed::create([
                ...
                'drive' => 'streamsb',
                'resolution' => '720p',
                'embed_link' => "https://sblongvu.com/e/".$code.".html"
            ]);
        }
    ...
    {
        $url = 'https://api.streamsb.com/api/upload/server?key='.$api_key;
