                ...
                <div class="w-full px-4">
                    <div
                        ...
                        <div class="flex-auto px-4 lg:px-10 py-10 pt-10">
                            ...
                            <form method="POST" action="{{ route('channel-server.update',$server->id) }}">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="channel_id" value="{{ $server->channel_id }}">
                                <div class="relative w-full mb-3">
                                    ...
                                    <input type="number"
                                        ...
                                        placeholder="Server Number" name="server_no" value="{{ old('server_no') !== null ? old('server_no') : $server->server_no }}"/>
                                </div>
                                <div class="relative w-full mb-3">
                                    <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2">Link</label>
                                    <input type="url"
                                        class="border-0 px-3 py-2 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                                        placeholder="Link" name="link" value="{{ old('link') !== null ? old('lnk') : $server->link }}" />
                                </div>
