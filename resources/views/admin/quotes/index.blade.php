        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0">Quote List</h4>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                                ...
                                <tbody>
                                    @foreach ($quotes as $item)
                                        <tr>
                                            ...
                                            <td>
                                                {{ Str::limit($item->text, 45) }}</td>
                                            <td><i class="{{ $item->published === 1 ? 'ri ri-checkbox-fill text-success' : 'bx bxs-x-circle text-danger' }}"
                                                    style="font-size: 25px;"></i>
                                            </td>
                                            <td>{{ $item->created_at->format('j F y') }}</td>
                                            <td>{{ $item->updated_at->format('j F y') }}</td>
                                            <td class="d-flex" style="font-size: 20px">
                                                <a href="{{ route('quotes.edit', $item->id) }}" class="me-2">
                            ...
                            {{ $quotes->onEachSide(2)->links('admin.partials._pagination') }}
                        </div>
                    </div>
                </div>
            </div>
